...
Validation Score (from notebook): 0.8949
"""
...
class GemmaXGBoost(BaselineModel):
    """
    Gemma seq128 embeddings + XGBoostClassifier baseline.
    Uses v4 (minimal) preprocessing - best for transformer-based embedders.
    
    XGBoostClassifier hyperparameters:
    ...
    Note: Uses CPU by default to avoid CUDA compatibility issues on heterogeneous clusters.
    """
    ...
    def get_model(self):
        """Factory method for fresh XGBClassifier instance."""
        # Use CPU by default; XGBoost will auto-detect GPU if available
        # This avoids CUDA compute capability mismatch errors on heterogeneous clusters
